...
def compute_result(strategy, length_of_memory):
    av, stddev, _min, _max, edges_added_av, = execute_sim(strategy, length_of_memory)

    print(f"{strategy} {length_of_memory} av: {av} stddev: {stddev} min: {_min} max: {_max} edges_added_av: {edges_added_av}")
